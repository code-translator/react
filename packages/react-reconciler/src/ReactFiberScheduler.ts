...
import { Batch, FiberRoot } from './ReactFiberRoot';
...
__interactionsRef,
__subscriberRef,
unstable_wrap as Scheduler_tracing_wrap } from
'scheduler/tracing';
import {
...
unstable_IdlePriority as IdlePriority } from
'scheduler';
import {
invokeGuardedCallback,
hasCaughtError,
clearCaughtError } from
'shared/ReactErrorUtils';
...
SimpleMemoComponent,
SuspenseComponent,
DehydratedSuspenseComponent } from
...
didExpireAtExpirationTime } from
...
stopCommitLifeCyclesTimer } from
'./ReactDebugFiberPerf';
...
computeInteractiveExpiration } from
...
popContext as popLegacyContext } from
'./ReactFiberContext';
...
recordCommitTime,
startProfilerTimer,
stopProfilerTimerIfRunningAndRecordDelta } from
'./ReactProfilerTimer';
import {
checkThatStackIsEmpty,
resetStackAfterFatalErrorInDev } from
'./ReactFiberStack';
      ...
      if (replayError != null && thrownValue != null) {
        try {
          ...
          if (replayError._suppressLogging) {
            ...
            (thrownValue as any)._suppressLogging = true;
          }
        } catch (inner) {
          // Ignore.
        }
...
function markLegacyErrorBoundaryAsFailed(instance: unknown) {
    ...
    try {
      ...
      if (nextUnitOfWork === null) {
        ...
        if (__DEV__) {
          ...
          (resetCurrentlyProcessingQueue as any)();
        }
...
function captureCommitPhaseError(sourceFiber: Fiber, value: unknown) {
...
function computeThreadID(expirationTime:
ExpirationTime, interactionThreadID:
number)
: number {
...
function renderDidSuspend(root:
FiberRoot, absoluteTimeoutMs:
number, suspendedTime:
ExpirationTime)
{
...
export function warnIfNotCurrentlyBatchingInDev(fiber: Fiber): undefined {
  if (__DEV__) {
...
function syncUpdates<A, B, C0, D, R>(fn:
(a: A, b: B, c: C0, d: D) => R, a:
A, b:
B, c:
C0, d:
D)
...
function scheduleCallbackWithExpirationTime(root:
...
function onSuspend(root:
FiberRoot, finishedWork:
Fiber, suspendedExpirationTime:
ExpirationTime, rootExpirationTime:
ExpirationTime, msUntilTimeout:
number)
: void {
...
function flushControlled(fn: () => unknown): void {
  const previousIsBatchingUpdates = isBatchingUpdates;
  isBatchingUpdates = true;
  try {
    syncUpdates(fn);
