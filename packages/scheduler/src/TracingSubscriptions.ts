...
let subscribers: Set<Subscriber> = (null as any);
if (enableSchedulerTracing) {
  subscribers = new Set();
}

export function unstable_subscribe(subscriber: Subscriber): undefined {
  if (enableSchedulerTracing) {
...
export function unstable_unsubscribe(subscriber: Subscriber): undefined {
  if (enableSchedulerTracing) {
...
function onWorkScheduled(interactions:
Set<Interaction>, threadID:
number)
: void {
  let didCatchError = false;
  let caughtError = null;
...
function onWorkCanceled(interactions:
Set<Interaction>, threadID:
number)
: void {
  let didCatchError = false;
