...
import { ReactProvider, ReactContext } from 'shared/ReactTypes';
...
enableSuspenseServerRenderer } from
...
Dispatcher,
currentThreadID,
setCurrentThreadID } from
'./ReactPartialRendererHooks';
import {
Namespaces,
getIntrinsicNamespace,
getChildNamespace } from
'../shared/DOMNamespaces';
...
const toArray = ((React.Children.toArray as any) as toArrayType);
...
if (__DEV__) {
  ...
  pushElementToDebugStack = function (element: ReactElement) {
    ...
    ((frame as any) as FrameDev).debugElementStack.push(element);
  ...
  getCurrentServerStackImpl = function (): string {
    ...
    for (let i = frames.length - 1; i >= 0; i--) {
      ...
      let debugElementStack = ((frame as any) as FrameDev).debugElementStack;
...
function warnNoop(publicInstance:
React$Component<any, any>, callerName:
string)
{
  if (__DEV__) {
...
function flattenTopLevelChildren(children: unknown): FlatReactChildren {
  ...
  const element = ((children as any) as ReactElement);
  ...
  const fragmentChildElement = ((fragmentChildren as any) as ReactElement);
  return [fragmentChildElement];
}

function flattenOptionChildren(children: unknown): string | null | undefined {
...
function createOpenTagMarkup(tagVerbatim:
string, tagLowercase:
string, props:
Object, namespace:
string, makeStaticMarkup:
...
class ReactDOMServerRenderer {
  ...
  contextProviderStack: Array<ReactProvider<any>> | null | undefined; // DEV-only
  ...
  constructor(children: unknown, makeStaticMarkup: boolean) {
    ...
    if (__DEV__) {
      ((topFrame as any) as FrameDev).debugElementStack = [];
    }
  ...
  pushProvider<T>(provider: ReactProvider<T>): undefined {
  ...
  popProvider<T>(provider: ReactProvider<T>): undefined {
    const index = this.contextIndex;
    if (__DEV__) {
      warningWithoutStack(
      index > -1 && provider === (this.contextProviderStack as any)[index],
      'Unexpected pop.');
    ...
    try {
      ...
      while (out[0].length < bytes) {
        ...
        if (__DEV__) {
          ...
          ((frame as any) as FrameDev).debugElementStack.length = 0;
        }
        try {
  ...
  : string {
    if (typeof child === 'string' || typeof child === 'number') {
      ...
      if (nextChild === null || nextChild === false) {
        ...
        if (__DEV__) {
          ((frame as any) as FrameDev).debugElementStack = [];
        }
        this.stack.push(frame);
        return '';
      }
      // Safe because we just checked it's an element.
      const nextElement = ((nextChild as any) as ReactElement);
      ...
      switch (elementType) {
        ...
        case REACT_FRAGMENT_TYPE:{
            ...
            ((nextChild as any) as ReactElement).props.children);
        ...
        case REACT_SUSPENSE_TYPE:{
            if (enableSuspenseServerRenderer) {
              const fallback = ((nextChild as any) as ReactElement).props.fallback;
              ...
              if (__DEV__) {
                ((frame as any) as FrameDev).debugElementStack = [];
                ((fallbackFrame as any) as FrameDev).debugElementStack = [];
              }
              this.stack.push(frame);
              this.suspenseDepth++;
              return '<!--$-->';
      ...
      if (typeof elementType === 'object' && elementType !== null) {
        switch (elementType.$$typeof) {
          case REACT_FORWARD_REF_TYPE:{
              const element: ReactElement = ((nextChild as any) as ReactElement);
              let nextChildren;
              const componentIdentity = {};
              prepareToUseHooks(componentIdentity);
