...
findHostInstance,
findHostInstanceWithWarning } from
'react-reconciler/inline.fire';
...
enqueueStateRestore,
restoreStateIfNeeded } from
'events/ReactControlledComponent';
import {
injection as EventPluginHubInjection,
runEventsInBatch } from
'events/EventPluginHub';
import { eventNameDispatchConfigs } from 'events/EventPluginRegistry';
import {
accumulateTwoPhaseDispatches,
accumulateDirectDispatches } from
'events/EventPropagators';
...
getClosestInstanceFromNode } from
'../client/ReactDOMComponentTree';
...
(Element & {
  _reactRootContainer: Root | null | undefined;
  _reactHasBeenPassedToCreateRootDEV: boolean | null | undefined;}) | (
...
type Root = {
  ...
  React$Component<any, any> | null | undefined, children:
  ReactNodeList, callback:
...
ReactBatch.prototype.then = function (onComplete: () => unknown) {
...
ReactWork.prototype.then = function (onCommit: () => unknown): undefined {
  if (this._didCommit) {
    onCommit();
    return;
  }
...
ReactWork.prototype._onCommit = function (): undefined {
  if (this._didCommit) {
    return;
  }
...
function legacyRenderSubtreeIntoContainer(parentComponent:
React$Component<any, any> | null | undefined, children:
ReactNodeList, container:
DOMContainer, forceHydrate:
boolean, callback:
...
const ReactDOM: Object = {
  ...
  findDOMNode(
  componentOrElement: Element | (React$Component<any, any> | null | undefined))
  : null | Element | Text {
    if (__DEV__) {
  ...
  hydrate(element: React$Node, container: DOMContainer, callback: Function | null | undefined) {
    invariant(
    isValidContainer(container),
    'Target container is not a DOM element.');
